...
const trust_network_1 = require("./src/trust-network");
let passed = 0;
let failed = 0;
...
test('Agent creation with default values', () => {
    const agent = new agent_1.Agent({ id: 'test-1', name: 'Test', behavior: 'neutral' });
...
test('Agent trust score bounds (0-1)', () => {
    const agent = new agent_1.Agent({ id: 'test-3', name: 'Test', behavior: 'neutral' });
...
test('Agent interaction recording', () => {
    const agent = new agent_1.Agent({ id: 'test-4', name: 'Test', behavior: 'cooperative' });
...
test('Agent cooperation decision', () => {
    const cooperative = new agent_1.Agent({ id: 'test-5', name: 'Coop', behavior: 'cooperative' });
    const adversarial = new agent_1.Agent({ id: 'test-6', name: 'Adv', behavior: 'adversarial' });
    const other = new agent_1.Agent({ id: 'test-7', name: 'Other', behavior: 'neutral' });
...
test('Agent task performance varies by behavior', () => {
    const cooperative = new agent_1.Agent({
        id: 'test-8',
        name: 'Coop',
        behavior: 'cooperative',
        reliability: 1.0
    });
    const adversarial = new agent_1.Agent({
        id: 'test-9',
        name: 'Adv',
    ...
    for (let i = 0; i < trials; i++) {
        if (cooperative.performTask(0))
            coopSuccess++;
        if (adversarial.performTask(0))
            advSuccess++;
    }
    // 合作型应该有更高的成功率
...
test('Network creation', () => {
    const network = new trust_network_1.TrustNetwork();
    const stats = network.getStats();
    return stats.totalAgents === 0 && stats.totalRelations === 0;
});
test('Add agents to network', () => {
    const network = new trust_network_1.TrustNetwork();
    const agent = new agent_1.Agent({ id: 'net-1', name: 'Agent 1', behavior: 'neutral' });
...
test('Remove agents from network', () => {
    const network = new trust_network_1.TrustNetwork();
    const agent = new agent_1.Agent({ id: 'net-2', name: 'Agent 2', behavior: 'neutral' });
    network.addAgent(agent);
...
test('Set trust relation', () => {
    const network = new trust_network_1.TrustNetwork();
    const agent1 = new agent_1.Agent({ id: 'net-3', name: 'Agent 3', behavior: 'neutral' });
    const agent2 = new agent_1.Agent({ id: 'net-4', name: 'Agent 4', behavior: 'neutral' });
...
test('Trust relation bounds (0-1)', () => {
    const network = new trust_network_1.TrustNetwork();
    const agent1 = new agent_1.Agent({ id: 'net-5', name: 'Agent 5', behavior: 'neutral' });
    const agent2 = new agent_1.Agent({ id: 'net-6', name: 'Agent 6', behavior: 'neutral' });
    network.addAgent(agent1);
    network.addAgent(agent2);
    network.setTrustRelation('net-5', 'net-6', 1.5);
    if (network.getTrustWeight('net-5', 'net-6') !== 1)
        return false;
...
test('Malicious agent detection', () => {
    const network = new trust_network_1.TrustNetwork();
    const goodAgent = new agent_1.Agent({
        id: 'good',
        name: 'Good',
        behavior: 'cooperative',
        initialTrust: 0.9
    });
    const badAgent = new agent_1.Agent({
        id: 'bad',
        name: 'Bad',
        behavior: 'malicious',
        initialTrust: 0.2
...
test('Trusted agent retrieval', () => {
    const network = new trust_network_1.TrustNetwork();
    const trusted = new agent_1.Agent({
        id: 'trusted',
        name: 'Trusted',
        behavior: 'cooperative',
        initialTrust: 0.9
    });
    const untrusted = new agent_1.Agent({
        id: 'untrusted',
        name: 'Untrusted',
        behavior: 'neutral',
        initialTrust: 0.5
...
test('Interaction simulation', () => {
    const network = new trust_network_1.TrustNetwork();
    const agent1 = new agent_1.Agent({
        id: 'sim-1',
        name: 'Agent 1',
        behavior: 'cooperative',
        reliability: 1.0
    });
    const agent2 = new agent_1.Agent({
        id: 'sim-2',
        name: 'Agent 2',
        behavior: 'cooperative',
        reliability: 1.0
...
test('Export and import', () => {
    ...
    const imported = trust_network_1.TrustNetwork.import(exported);
