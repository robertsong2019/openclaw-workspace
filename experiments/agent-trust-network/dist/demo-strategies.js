 ...
 */
function playPrisonersDilemma(agent1, agent2, rounds = 100) {
    let score1 = 0;
    let score2 = 0;
    let cooperations1 = 0;
    let cooperations2 = 0;
    for (let i = 0; i < rounds; i++) {
        // 决定是否合作
        const cooperate1 = agent1 instanceof advanced_agent_1.AdvancedAgent
            ? agent1.shouldCooperateWith(agent2)
            : agent1.shouldCooperate(agent2);
        const cooperate2 = agent2 instanceof advanced_agent_1.AdvancedAgent
        ...
        // 记录策略交互（用于有记忆的策略）
        if (agent1 instanceof advanced_agent_1.AdvancedAgent) {
            agent1.recordStrategicInteraction(agent2.id, cooperate2);
        }
        if (agent2 instanceof advanced_agent_1.AdvancedAgent) {
 ...
 * 运行锦标赛
 */
function runTournament(strategies, roundsPerMatch = 200) {
...
const strategies = [
    {
        name: 'Tit-for-Tat',
        create: () => new advanced_agent_1.AdvancedAgent({
            id: 'tft',
            name: 'Tit-for-Tat',
...
console.log('');
// 找出最合作的策略
const mostCooperative = tournamentResults.reduce((a, b) => a.cooperationRate > b.cooperationRate ? a : b);
...
// 找出最自私的策略
const leastCooperative = tournamentResults.reduce((a, b) => a.cooperationRate < b.cooperationRate ? a : b);
...
console.log('╚════════════════════════════════════════════════════════════╝');
//# sourceMappingURL=demo-strategies.js.map